...
public class RuleManager {
    ...
    public void addRule(Rule<Integer> r) {
        if (!rules.contains(r)) {
            rules.add(r);
            if (r.isBinary()) {
                int action = DataInfo.instance().getUpdateNumericAction(DataInfo.REDUCE + "_" + DataInfo.instance().label(r.cons())); // reduce action
                r.setAction(action);
                ...
                if (DataInfo.instance().isPreTerminal(unary)) { // unary child is a preterminal
                    action = DataInfo.instance().getUpdateNumericAction(DataInfo.SHIFT + "_" + DataInfo.UNARY + "_" + DataInfo.instance().label(r.cons())); // shu action
                } else { // unary child is a constituent
                    action = DataInfo.instance().getUpdateNumericAction(DataInfo.REDUCE + "_" + DataInfo.instance().label(unary) + "_" + DataInfo.UNARY + "_" + DataInfo.instance().label(r.cons())); // shu action
                }
                r.setAction(action);
